...
class Main:
    def __init__(self):
        ...
        # labelのスタイル
        style.configure("Custom.TLabel", foreground="black", font=("游明朝"))
        ...
        self.regions_combobox = ttk.Combobox(menu_frame, values=self.region_keys, width=10, state="readonly")
        ...
        display_area = ttk.Label(forecast_frame, textvariable=self.area, style="Custom.TLabel")
        ...
        self.weather_tree.column("date", width=55)
        self.weather_tree.column("area", width=105)
        self.weather_tree.column("weather", width=500)
        self.weather_tree.column("wind", width=300)
    ...
    def change_region(self, event):
        ...
        セレクトボックスで地方を選択する
        """
        self.prefectures_combobox["values"] = list(self.regions[event.widget.get()].keys())
        self.prefectures_combobox.set("----------")
